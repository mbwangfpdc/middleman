...
fn get_input_as_u8() -> u8 {
    let stdin = io::stdin();
    return stdin.lock().lines().next().unwrap().unwrap().parse::<u8>().unwrap();
}

fn update_positions_and_moves(possible_moves: &mut HashSet<u8>, positions: &mut HashSet<u8>, position: &u8) {
